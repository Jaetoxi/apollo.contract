...
#define CHECKC(exp, code, msg) \
   ...
   void nftone_mart::on_buy_transfer(const name& from, const name& to, const asset& quant, const string& memo) {
      ...
      uint64_t needamount = (order.price.value.amount + order.fee.amount) * count;
      CHECKC( needamount > 0, err::PARAM_ERROR, "non-positive count not allowed" )
      CHECKC( quant.amount == needamount, err::INCORRECT_AMOUNT, "incorrect amount" )
